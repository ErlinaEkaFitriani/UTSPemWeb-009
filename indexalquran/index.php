...
@font-face {
  font-family: 'Uthmani';
  src : url('http://localhost/indexindexalquran/sources/font/UthmanicHafs1 Ver09.otf') format('truetype');
}
...
function show_daftar(){
	...
	foreach($data as $d){
		echo '<tr><td>'.$d['index'].'</td><td><a href="http://localhost/indexalquran/index.php?surat='.$d['index'].'&nama='.$d['surat_indonesia'].'">'.$d['surat_indonesia'].'</a></td><td>'.$d['arti'].'</td></td><td>'.$d['jumlah_ayat'].'</td></tr>';
	}
	echo '</table>';
}
function show_quran($surat, $nama=''){	
	...
	echo '<p><a href="http://localhost/indexalquran">Kembali ke Index</a></p>';
	echo '<h3>'.$nama.'</h3>';
	if($surat > 1) {
